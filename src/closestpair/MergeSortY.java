...
public class MergeSortY {
	...
	private DataPoint[] MergeSortGo(DataPoint[] dp_arr, int length){
		if(dp_arr.length < 2){
			return dp_arr;
		}
		int new_length = dp_arr.length/2;
	...
	private DataPoint[] Merge(DataPoint[] dp_arr_left,DataPoint[] dp_arr_right){
		int left_holder = 0;
		int right_holder = 0;
	...
	private DataPoint[] copyDP(DataPoint[] dp_arr, DataPoint[] newGuy, int begin, int end){
		int marker = 0;
